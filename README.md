...
**View Logs:**
```bash
docker logs $(docker ps -q --filter ancestor=scsibug/nostr-rs-relay)
```

### Relay Details
- **URL**: `ws://localhost:7000` (for Nostr clients)
- **HTTP**: `http://localhost:7000` (for web interface)
- **Database**: Persistent Docker volume `nostr-data`
- **Configuration**: Custom `config.toml` with proper settings
